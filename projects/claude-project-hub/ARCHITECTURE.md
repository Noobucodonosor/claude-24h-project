...
**Write Operations**: Direct commits via GitHub API (code, PROJECT.md updates)  
**Versioning**: Full Git history for all changes

**Key Feature**: Code generated by Claude → Immediate GitHub commit (no download/upload)
...
#### **Tier 3: Browser Cache (Session Only)**
```typescript
// Temporary session cache (NOT persistent storage)
const sessionCache = {
  currentProject: null,      // Active project context
  conversationBuffer: [],    // Last 10 messages (performance)
  apiResponses: new Map()    // Dedupe API calls
};

// NO IndexedDB for permanent storage
// Everything persists to GitHub/Supabase
```

**Cache Strategy**:
```
Session Active:
- Keep conversation in memory (RAM)
- Cache API responses (5 min TTL)
- Auto-save to Supabase every 30 seconds

Session Ends:
- Flush conversation to Supabase
- Clear browser cache
- Next session: Load from Supabase + GitHub
```

**No Offline Mode**: Always requires 5G/WiFi connection. If offline, show: "Reconnect to continue."
